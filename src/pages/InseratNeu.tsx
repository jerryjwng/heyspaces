...
function AIAssistantModal({
  ...
  const generate = () => {
    setState('loading');
    // TODO: Replace mock with real Claude API call in Phase 2
    setTimeout(() => {
    ...
    <>
      ...
      <div className="fixed left-1/2 top-1/2 z-[51] w-[calc(100%-32px)] max-w-[560px] -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-[24px] bg-white p-8 shadow-2xl max-h-[90vh]">
        ...
        <div className="flex items-start justify-between">
          ...
          <button
            onClick={handleClose}
            className="absolute right-6 top-6 flex h-8 w-8 items-center justify-center rounded-full bg-[#F5F5F3] text-[#6B6B6B] transition-colors hover:bg-[#EBEBEB]"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div
          ...
          className="cursor-pointer rounded-[16px] border-2 border-dashed border-[#D4D4D0] bg-[#F5F5F3] p-6 text-center transition-colors hover:border-[#0C0C0C] hover:bg-[#EBEBEB]"
        >
            ...
            {files.map((f, i) => (
              <div key={i} className="flex items-center justify-between rounded-[10px] border border-[#EBEBEB] bg-white px-3.5 py-2.5">
                ...
                <button
                  ...
                  className="text-[#A8A8A8] hover:text-[#0C0C0C]"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
        ...
        <textarea
          ...
          placeholder="Was ist besonders an dieser Wohnung? z.B. renoviert 2023, ruhige Innenlage, Einbauküche vorhanden..."
        ...
        <div className="mt-6">
          {state === 'idle' && (
            <button
              onClick={generate}
              className="flex w-full items-center justify-center gap-2 rounded-full bg-[#0C0C0C] px-4 py-4 text-[15px] font-semibold text-white transition-all hover:bg-[#2A2A2A] active:scale-[0.99]"
            >
          ...
          {state === 'success' && (
            <div className="rounded-[16px] bg-[#E8F5EE] p-5">
              ...
              <p className="mt-2 line-clamp-3 text-[13px] text-[#6B6B6B]">
                {generated.slice(0, 180)}...
              </p>
              <div className="mt-4 flex gap-2.5">
                <button
                  ...
                  className="rounded-full bg-[#0C0C0C] px-6 py-2.5 text-[14px] font-semibold text-white transition-colors hover:bg-[#2A2A2A]"
                >
                  Übernehmen
                </button>
                <button
                  onClick={reset}
                  className="rounded-full border-[1.5px] border-[#EBEBEB] px-6 py-2.5 text-[14px] text-[#6B6B6B] transition-colors hover:border-[#0C0C0C] hover:text-[#0C0C0C]"
                >
                  Neu generieren
                </button>
              </div>
  ...
  if (published) {
    return (
      <div className="min-h-screen bg-[#FAFAF8]">
        <Navbar />
        <div className="mx-auto flex max-w-[680px] flex-col items-center px-6 py-20 text-center">
          <CheckCircle2 className="h-14 w-14 text-[#1A6B3C]" />
          <h1 className="mt-6 text-[28px] font-bold tracking-[-0.3px] text-[#0C0C0C]">
            Dein Inserat ist jetzt live!
          </h1>
          <p className="mt-2 text-[15px] text-[#6B6B6B]">
            Es wird nun auf HeySpaces angezeigt.
          </p>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <button
              onClick={() => navigate('/inserate/1')}
              className="rounded-full bg-[#0C0C0C] px-8 py-3.5 text-[15px] font-semibold text-white transition-colors hover:bg-[#2A2A2A]"
            >
              Inserat ansehen
            </button>
            <button
              onClick={() => navigate('/dashboard')}
              className="rounded-full border-[1.5px] border-[#EBEBEB] px-8 py-3.5 text-[15px] font-semibold text-[#0C0C0C] transition-colors hover:border-[#0C0C0C]"
            >
              Zum Dashboard
            </button>
          </div>
      ...
      <div className="mx-auto max-w-[680px] px-6 pb-20 pt-12 md:px-6">
          ...
          <div className="animate-fade-in">
                ...
                {KATEGORIEN.map(({ value, label, Icon }) => {
                  ...
                  return (
                    <button
                      key={value}
                      type="button"
                      onClick={() => setKategorie(value)}
                      className={cn(
                        'rounded-[16px] border-[1.5px] p-5 text-center transition-all',
                        selected
                          ? 'border-[#0C0C0C] bg-[#FAFAF8]'
                          : 'border-[#EBEBEB] bg-white hover:border-[#D4D4D0]',
                      )}
                    >
                      <Icon
                        className={cn(
            ...
            <div className="mt-5">
              <div className="mb-3 flex items-center justify-between">
                <span className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[#A8A8A8]">
                  BESCHREIBUNG
                </span>
                <button
                  onClick={() => setAiOpen(true)}
                  className="flex items-center gap-1.5 rounded-full bg-[#0C0C0C] px-3.5 py-1.5 text-[12px] font-semibold text-white transition-colors hover:bg-[#2A2A2A]"
                >
              ...
              <textarea
                ...
                className={cn(
                  ...
                  aiGenerated && 'border-[#1A6B3C] bg-white',
                )}
              />
              {aiGenerated && (
                <button
                  onClick={() => setAiOpen(true)}
                  className="mt-2 text-[12px] text-[#6B6B6B] hover:text-[#0C0C0C] hover:underline"
                >
                  Neu generieren
                </button>
              )}
            ...
            <div className="mt-5">
              ...
              <div className="flex gap-3">
                ...
                <select
                  value={preisUnit}
                  onChange={e => setPreisUnit(e.target.value as 'monat' | 'kauf')}
                  className={cn(inputCls, 'w-[160px] cursor-pointer')}
                >
          ...
          <div className="animate-fade-in">
            ...
            <div className="mt-10 grid grid-cols-1 gap-5 sm:grid-cols-2">
              ...
              <div>
                ...
                <div className="flex items-center justify-between rounded-[10px] bg-[#F5F5F3] px-4 py-3">
                  <button
                    onClick={() => setZimmer(z => Math.max(0.5, +(z - 0.5).toFixed(1)))}
                    className="flex h-7 w-7 items-center justify-center rounded-full text-[#0C0C0C] hover:bg-[#EBEBEB]"
                  >
                    <Minus className="h-4 w-4" />
                  </button>
                  <span className="text-[15px] font-semibold text-[#0C0C0C]">{zimmer}</span>
                  <button
                    onClick={() => setZimmer(z => +(z + 0.5).toFixed(1))}
                    className="flex h-7 w-7 items-center justify-center rounded-full text-[#0C0C0C] hover:bg-[#EBEBEB]"
                  >
            ...
            <div className="mt-5">
              ...
              <input
                type="date"
                value={verfuegbar}
                onChange={e => setVerfuegbar(e.target.value)}
                className={inputCls}
              />
            </div>
            ...
            <div className="mt-8 flex justify-between">
              <button
                onClick={() => setStep(1)}
                className="rounded-full border-[1.5px] border-[#EBEBEB] px-6 py-3.5 text-[15px] font-semibold text-[#6B6B6B] transition-colors hover:border-[#0C0C0C] hover:text-[#0C0C0C]"
              >
                ← Zurück
              </button>
              <button
        ...
        {step === 3 && (
          <div className="animate-fade-in">
            ...
            <p className="mt-2 text-[15px] text-[#6B6B6B]">
              Gute Fotos erhöhen deine Chancen auf Vermietung.
            </p>

            <div
              onClick={() => photoRef.current?.click()}
              className="mt-10 cursor-pointer rounded-[20px] border-2 border-dashed border-[#D4D4D0] bg-[#F5F5F3] px-8 py-12 text-center transition-colors hover:border-[#0C0C0C] hover:bg-[#EBEBEB]"
            >
              ...
              <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3">
                {photos.map((p, i) => (
                  <div key={i} className="group relative aspect-[4/3] overflow-hidden rounded-[12px] bg-[#F5F5F3]">
                    ...
                    {i === 0 && (
                      <span className="absolute left-2 top-2 rounded-full bg-[#0C0C0C] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-white">
                        Titelbild
                      </span>
                    )}
                    <button
                ...
                {photos.length === 0 && (
                  <div className="col-span-full flex aspect-[4/3] items-center justify-center text-[#A8A8A8]">
            ...
            <div className="mt-8 flex justify-between gap-3">
              ...
              >
                ...
                {publishing ? 'Veröffentliche...' : 'Inserat veröffentlichen'}
              </button>
            </div>
          </div>
        )}
